...
describe('Review', () => {
  ...
  describe('isTrustworthy', () => {
    test('コメントありの場合、評価とコメントの品質を組み合わせて信頼性を判断する', () => {
      const review = Review.create(reviewIdentity, bookId, name, rating, comment);
    ...
    test('コメントなしの場合、評価の品質のみで信頼性を判断する', () => {
  ...
  describe('extractRecommendedBooks', () => {
    ...
    test('コメントからパターンに一致する複数の推薦本を抽出できる', () => {
      ...
      // 両方のパターンに一致するはず
      expect(result).toEqual(['実践ドメイン駆動設計', 'エリック・エヴァンスのドメイン駆動設計']);
    });
