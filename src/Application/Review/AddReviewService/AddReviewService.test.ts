...
describe('AddReviewService', () => {
  ...
  test('存在する書籍に対してレビューを追加することができる', async () => {
    const bookId = '9784798126708';
    const book = Book.create(
      new BookIdentity(new BookId(bookId), new Title('テスト書籍'), new Author('テスト著者')),
      new Price({
        amount: 1500,
        currency: 'JPY',
      }),
  ...
  test('書籍が存在しない場合エラーを投げる', async () => {
    ...
    await expect(addReviewService.execute(command)).rejects.toThrow('書籍が存在しません');
  });
});
