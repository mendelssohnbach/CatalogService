...
import { DeleteReviewCommand, DeleteReviewService } from './DeleteReviewService';
...
describe('DeleteReviewService', () => {
  ...
  test('存在するレビューを削除することができる', async () => {
  ...
  test('レビューが存在しない場合エラーを投げる', async () => {
    const command: DeleteReviewCommand = {
      reviewId: 'non-existent-review',
    };

    await expect(deleteReviewService.execute(command)).rejects.toThrow('レビューが存在しません');
  });
});
