...
describe('SQLReviewRepository', () => {
  ...
  describe('save/findById', () => {
    test('saveしたレビューがfindByIdで取得できる', async () => {
      const review = createSampleReview('review-1', '山田太郎', 5, 'とても良い本でした。');
  ...
  describe('update', () => {
    test('updateで既存のレビューを更新できる', async () => {
      const review = createSampleReview('review-2', '佐藤次郎', 4, 'まあまあでした。');
      await reviewRepository.save(review);
    ...
    test('存在しないレビューのupdateでエラーが発生する', async () => {
      ...
      await expect(reviewRepository.update(nonExistingReview)).rejects.toThrow();
    });
  });

  describe('delete', () => {
    test('deleteで既存のレビューを削除できる', async () => {
      const review = createSampleReview('review-3', '鈴木三郎', 3, '普通でした。');
      await reviewRepository.save(review);
  ...
  describe('findAllByBookId', () => {
    test('findAllByBookIdで特定書籍のレビュー全てを取得できる', async () => {
      // 同じ書籍に複数のレビューを作成
      const review1 = createSampleReview('review-4', '田中四郎', 5, '最高でした！');
      const review2 = createSampleReview('review-5', '高橋五郎', 4, '良かったです。');
      const review3 = createSampleReview('review-6', '伊藤六郎', 3, 'まあまあでした。');
